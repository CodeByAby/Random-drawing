...
import { DeepAIClient } from "@/aisdk/deepai";
  ...
  try {
    ...
    switch (provider) {
      ...
      case "kling":
        ...
        break;
      case "deepai":
        // DeepAI uses different approach, handle separately
        const deepaiClient = new DeepAIClient({
          apiKey: process.env.DEEPAI_API_KEY || "",
        });
        
        try {
          const result = await deepaiClient.generateImage(prompt, model);
          
          // Convert DeepAI response to expected format
          const response = await fetch(result.output_url);
          const imageBuffer = await response.arrayBuffer();
          
          const storage = newStorage();
          const batch = getUuid();
          const filename = `deepai_image_${batch}_0.png`;
          ...
          console.log("DeepAI generation failed:", err);
          return respErr("DeepAI image generation failed");
        }
      default:
